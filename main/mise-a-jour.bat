...
cd %back%\temp
if exist id_rsa (echo clé SSH priveée  [42;37m[OK] [40;33m) else (echo [41;37m[ERREUR] [40;33mIl manque clé SSH)

if exist id_rsa.pub (echo clé SSH publique    [42;37m[OK] [40;33m) else (echo [41;37m[ERREUR] [40;33mIl manque clé SSH)
...
rem instalation des clée ssh
cd C:\Program Files (x86)\Git\etc\ssh

if exist id_rsa cd %back%temp & mkdir backup-ssh & copy /y C:\Program Files (x86)\Git\etc\ssh %back%temp\backup-ssh\ 
if exist id_rsa.pub cd %back%temp & mkdir backup-ssh & copy /y C:\Program Files (x86)\Git\etc\ssh %back%temp\backup-ssh\ 
if %errorlevel% == 0 (echo ssh [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh manquant & mkdir .ssh)

copy /y %back%\temp\id_rsa.pub "C:\Program Files (x86)\Git\etc\ssh"
if %errorlevel% == 0 (echo ssh exporter [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh pas exporter & mkdir .ssh)
copy /y %back%\temp\id_rsa "C:\Program Files (x86)\Git\etc\ssh"
if %errorlevel% == 0 (echo ssh exporter [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh pas exporter & mkdir .ssh)
...
git clone git@github.com:00MY00/version.git --progress
...
mkdir scripts
cd %USERPROFILE%
if not exist scripts git clone git@github.com:00MY00/scripts.git --progress
...
if %errorlevel% == 1 set ERREUR=1
@echo ^<link href="resourc-img\style.css" rel="stylesheet"^> >> t.hta
if %errorlevel% == 1 set ERREUR=1
@echo ^<html^> >> t.hta
if %errorlevel% == 1 set ERREUR=1
@echo   ^<head^> >> t.hta
...
if %python% == 1 if exist %USERPROFILE%\AppData\Local\Programs\Python\Python* cd %USERPROFILE%\AppData\Local\Programs\Python\Python* & python --version & if %errorlevel% == 0 timeout /t 40 /nobreak &  set python=0 & goto start

if %Git% == 1 if exist "C:\Program Files (x86)\Git" git --version & if %errorlevel% == 0 (timeout /t 40 /nobreak & set Git=0 & set PATH=%PATH%;C:\Program Files (x86)\Git\cmd & set PATH=%PATH%;C:\Program Files (x86)\Git & goto	start) else	(goto extraction-resourc-prog)
