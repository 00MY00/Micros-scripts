...
if exist id_rsa cd %back%temp & mkdir backup_ssh & cd %USERPROFILE%\.ssh\ & copy /y %USERPROFILE%\.ssh\ %back%temp\backup_ssh\
if exist id_rsa.pub cd %back%temp & mkdir backup_ssh & cd %USERPROFILE%\.ssh\ & copy /y %USERPROFILE%\.ssh\ %back%temp\backup_ssh\
if %errorlevel% == 0 (echo ssh [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh manquant & mkdir .ssh)

copy /y %back%\temp\id_rsa.pub %USERPROFILE%\.ssh\
if %errorlevel% == 0 (echo ssh exporter [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh pas exporter & mkdir .ssh)
copy /y %back%\temp\id_rsa %USERPROFILE%\.ssh\
if %errorlevel% == 0 (echo ssh exporter [42;37m[OK]  [40;33m) else (echo 41;37m[ERREUR] [40;33mssh pas exporter & mkdir .ssh)
